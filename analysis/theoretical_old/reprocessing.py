...
labels['OIIIHbeta_EW'] = r'$\rm \log_{10}(EW_{\regular{H\beta+[OIII]4959,5007}}/{\regular \AA})$'
...
colors = cmr.take_cmap_colors('cmr.pride', 3, cmap_range=(0.3, 0.7))
lw1, lw2 = [1,2]
alpha1, alpha2 = [1, 0.5]
ls1, ls2 = ['-','--']
...
Hbeta_EW = np.log10(data['Hbeta_EW'])
OIIIHbeta_EW = np.log10(data['OIIIHbeta_EW'])

# for k, d, c in zip(['BB', 'beta', 'HbetaEW'], [BB, beta, HbetaEW], colors):
#     ax.plot(x, (d - d[0]), c=c, ls=ls1, lw=lw1, alpha = alpha1, label = labels[k])

for k, d, c in zip(['BB', 'beta', 'OIIIHbeta_EW'], [BB, beta, Hbeta_EW], colors):
    ax2.plot(x, (d - d[0]), c=c, ls=ls1, lw=lw1, alpha = alpha1, label = labels[k])


ax2.set_xlim([-2, 1])
ax2.set_ylim(-1.5, 1.5)
ax2.set_xlabel(r'$\rm log_{10}(\tau_V)$')
...
for k, d, c in zip(['BB', 'beta', 'OIIIHbeta_EW'], [BB, beta, OIIIHbeta_EW], colors):
    ax.plot(x, (d - d[0]), c=c, ls=ls2, lw=lw2, alpha = alpha2)
