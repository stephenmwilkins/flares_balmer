...
log10Zs = [-4, -3, -2]
colors = cmr.take_cmap_colors('cmr.guppy', len(log10Zs), cmap_range=(0.25, 0.75))
...
for log10Z, c in zip(log10Zs, colors):
    ...
    for i, sfh_type in enumerate(['exp100', 'constant', 'exp-100', 'instant']):
        ...
        # HbetaEW = np.log10(data['Hbeta_EW'])

        OIIIHbetaEW = np.log10(data['OIIIHbeta_EW'])

        for ax, d in zip(axes, [BB, beta, OIIIHbetaEW]):
...
limits['BB'] = [-0.3, 0.9]
limits['beta'] = [-3.5, 0.9]
limits['Hbeta_EW'] = [2.9, 0.1]
limits['OIIIHbeta_EW'] = [0.1, 3.5]

labels = {}
labels['BB'] = r'$\rm \log_{10}(L_{4200}/L_{3500})$'
labels['beta'] = r'$\rm \beta$'
labels['Hbeta_EW'] = r'$\rm \log_{10}(H\beta\ EW/\AA)$'
labels['OIIIHbeta_EW'] = r'$\rm \log_{10}(EW_{\regular{H\beta+[OIII]4959,5007}}/\AA)$'



# for ax, d in zip(axes, ['BB','beta','Hbeta_EW']):
for ax, d in zip(axes, ['BB','beta','OIIIHbeta_EW']):
