...
for i, sfh_type in enumerate(['constant','instant','exp100']):
    ...
    for j, log10tau_V in enumerate([-5., -1.0, -0.3, 0.0]):
        ...
        ax.plot(HbetaEW, C, c=c, lw=1, ls=ls, label = fr'$\rm \tau_{{V}}={tau_V}$')


    if sfh_type == 'constant':

        for fesc, c in zip([0.5, 0.9], ['0.6','0.8']):

            # add f_esc = 1.0
            data = ascii.read(f'data/{sfh_type}_fesc{fesc}_log10Z{log10Z}_tauV0.0.dat')
            # C = -2.5*np.log10(data['FAKE.FAKE.BBa']/data['FAKE.FAKE.BBb'])
            C = np.log10(data['FAKE.FAKE.BBb']/data['FAKE.FAKE.BBa'])
            HbetaEW = np.log10(data['HbetaEW'])
            ax.plot(HbetaEW, C, c=c, lw=1, ls=ls, label = rf'$\rm f_{{esc}}={fesc}$')
...
handles += [Line2D([0], [0], label = fr'$\rm f_{{esc}}=0.9$' , color='0.8', lw=1, ls='-')]
handles += [Line2D([0], [0], label = fr'$\rm f_{{esc}}=0.5$' , color='0.6', lw=1, ls='-')]
...
fig.savefig(f'figs/BB_HbetaEW.pdf')
fig.clf()
