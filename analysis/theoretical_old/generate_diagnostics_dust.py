...
for sfh_type in ['exp-100', 'exp100', 'constant', 'instant']:
    ...
    d_ = [log10tau_Vs]+[d[f] for f in filters]+[d[l] for l in ['Hbeta_EW','OIIIr_EW','OIIIb_EW','OIIIHbeta_EW']]
    names = ['log10tau_V']+filters+['Hbeta_EW','OIIIr_EW','OIIIb_EW','OIIIHbeta_EW']
    print(len(d_))
    print(len(names))
    data = Table(d_, names=names)
